...
class event_trigger:
    ...
    def SWA_CurWarning(self, dongle, bMask):
        ...
        for vol in range(110, 30, -1):
            ...
            data1 = bm.get_bit(data1, 's', [7, 3])
            print(data[0], data1[0])
            if data[0] == '0' and data1[0] == '1':
                el1.off([1])
    ...
    def SWA_CurLimit(self, dongle, bMask):
        ...
        # input('Please Connect DPS CH3 on VOUTA')
        a.dsh.i2c_write_reg(0x1C, 0xFF)  # maxmize current warning
        rgtset(dongle, 0x15, 1, 5)  # Mask SWA PG
        rgtset(dongle, 0x18, 1, 3)  # Mask SWA UV
        a.enable(True)
        time.sleep(1)
        # p1.SS(1.1, 0.9, 1, 3)
        for vol in range(90, 10, -1):
            # p1.on(vol / 100, 2, 1)
            time.sleep(0.2)
            el1.cv(vol / 100, 4.9, 1)
            el1.on([1])
            time.sleep(0.5)
            ret, data = dongle.dsh.i2c_read_reg(0x0B)
    ...
    def SWB_CurLimit(self, dongle, bMask):
        ...
        rgtset(dongle, 0x18, 1, 1)  # Mask SWB UV
        rgtset(dongle, 0x26, 1, 2)  # Lowest SWB UV threshold
        a.enable(True)
        time.sleep(1)
        # p1.SS(1.1, 0.9, 1, 3)
        for vol in range(90, 10, -1):
    ...
    def SWC_CurWarning(self, dongle, bMask):
        ...
        a.dsh.i2c_write_reg(0x20, 0xFF)  # maxmize current limit threshold
        a.dsh.i2c_write_reg(0x1F, 0x10)  # set SWC current warning 0.5A
        a.enable(True)
        time.sleep(1)
        for vol in range(185, 30, -1):
            el1.cv(vol / 100, 5, 4)
            el1.on([3])
            time.sleep(0.5)
            ret, data = dongle.dsh.i2c_read_reg(0x0B)
    ...
    def SWC_CurLimit(self, dongle, bMask):
        ...
        rgtset(dongle, 0x18, 1, 0)  # Mask SWC UV
        p1.on(1.8, 1, 3)
        a.enable(True)
        time.sleep(1)
        time.sleep(0.3)
    ...
    def SWB_OV(self, dongle, bMask):
        ...
        rgtset(dongle, 0x15, 1, 3)
        input('Please Connect DPS CH3 on VOUTB')
        p1.on(1.1, 1, 3)
        a.enable(True)
        time.sleep(1)
        rgtset(dongle, 0x82, 0, 1)  # disable Soft_OV
    ...
    def LDO1p0PG(self, dongle, bMask):
        ...
        el1.on([4])
        time.sleep(1)
        # for vol in range(100, 30, -1):
        #     el1.cv(vol / 100, 5, 4)
        #     el1.on([4])
        #     time.sleep(0.005)
        #     ret, data = dongle.dsh.i2c_read_reg(0x33)
        #     data = bm.get_bit(data, 's', [2])
        #     if data == '1':
        #         print('LDO1.0 PG found')
        #         break
        el1.off([4])
        global name
        name = sys._getframe().f_code.co_name
...
for i in range(5, 6, 1):
    ...
    if i == 0:
        ...
        c.clear_SWA_CurLimit(a)

    elif i == 4:
        c.clear_SWA_CurWarning(a)

    elif i == 5:
        c.clear_SWB_CurLimit(a)

    elif i == 6:
        c.clear_SWB_CurWarning(a)
