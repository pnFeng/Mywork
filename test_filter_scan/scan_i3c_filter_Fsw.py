...
handle = logging.FileHandler('I3C_filter_fsw_test_{}.log'.format(logname))
...
# dut.CFGR_PEC_ENABLE.i3c_set(dut)
...
scan_i3c_freq = [1000, 4000, 6000, 8000, 10000, 11000, 12000,
...
for Fsw_setting in scan_Fsw:
    ...
    for reg, setting in Fsw_setting:
        # filter.i3c_set()
        dut.i3c_write_reg(reg, setting)
        fsw_key += "{:02X}:{:02X};".format(reg, setting)
    record_dict[fsw_key] = {}

    dut.enable_vr(True, "I3C")
    time.sleep(0.5)

    for volt, freq in itertools.product(scan_volt, scan_i3c_freq):
        cur_key = "{:.01f}V / {}".format(volt, freq)
        dut.set_voltage(volt)
        dut.set_i3c_bitrate(freq, 1000)

        print("Filter:{}, Volt: {:.01f}, Freq:{} ".format(fsw_key, volt, freq))
        logger.warning("Filter:{}, Volt: {:.01f}, Freq:{} ".format(fsw_key, volt, freq))
    ...
    dut.enable_vr(False, "I3C")
    time.sleep(0.5)

df = pd.DataFrame.from_dict(record_dict)
df.T.to_excel('I3C_filter_fsw_test_{}.xlsx'.format(logname))
